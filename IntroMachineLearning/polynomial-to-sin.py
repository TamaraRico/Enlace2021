...
y = torch.sin(x)
# Tensors : a, b, c, d
# A Tensor represents a node in a computational graph. 
...
# Y = a + bx + cx^2 + dx3
# se inicializa cada valor del polinomios en valores random
...
# funcion que maneja el programa
    ...
    # loss representa la diferencia entre el valor que estamos obteniendo y el valor que debemos obtener
    if t % 100 == 99:
        print(t, loss.item())
    # si el numero es cercano lo imprime

    # calculamos e imprimimos loss para saber cómo el programa esta arreglando ese problrema

    # Use autograd to compute the backward pass. This call will compute the
    # gradient of loss with respect to all Tensors with requires_grad=True.
    # After this call a.grad, b.grad. c.grad and d.grad will be Tensors holding
    # the gradient of the loss with respect to a, b, c, d respectively.
    loss.backward() # backpropagation / retroalimentacion
    # esta funcion es la forma en la que la maquina esta viendo como estan saliendo los resultado y como deberian ajustarlos
    ...
    with torch.no_grad():
        ...
        c -= learning_rate * c.grad # usamos el menos porque nos podemos mover a ambos lados de la parabola, izq o derecha
        d -= learning_rate * d.grad # el learning rate es que tanto queremos que cambie el valor de las variables abcd

        # Manually zero the gradients after updating weights
        a.grad = None # reset the gradient for next iteration
        b.grad = None
        c.grad = None
        d.grad = None
